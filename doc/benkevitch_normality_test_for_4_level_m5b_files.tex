...
\noindent 0: no errors; \\
\noindent 1: bad frame signature; flags for all the channels are filled with 1.  \\
\noindent 2: 1D search for the optimal quantization threshold did not converge in 20 iterations; \\
\noindent 3: $\chi^2 > \chi^2_{cr}$.
...
Fig.~\ref{chi2_and_thresh_distr_1} and Fig.~\ref{chi2_and_thresh_distr_2} show example plots generated by \\
...
\subsection{Spotting Bad Frames}

Sometimes a few channels in good data are flagged just because of random fluctuations in the data itself, and not because an alien piece of code was injected. Normally, in such cases $\chi^2$ only slightly exceeds its critical value $\chi^2_{cr} = 7.81$. Other than that, a really foreign piece of code or data damages the whole frame or many frames in a row, and the $\chi^2$ values in such bad frames are many times larger than $\chi^2_{cr}$. In order to spot such frames we should roughen the Pearson's $\chi^2$ criterion by searching for frames where, say, $\chi^2 > 20$ or even 50. It is easy to do in \verb@IPython@. For example: \\

\noindent \verb@import numpy as np@ \\
\noindent \verb@c2 = np.fromfile("nt_chi2_cuda_rd1910_wz_268-1811_text_20231024_174438.173.bin",@ \\
\indent \indent \indent \indent \verb@dtype=np.float32)@ \\
\noindent \verb@c = c2.reshape((len(c2)//16,16))@ \\
\noindent \verb@ixc = np.where(c > 50)@ \\
\noindent \verb@for i in range(len(ixc[0])):@ \\
\indent \indent \verb@print(ixf[0][i], ixf[1][i], c[ixf[0][i], ixf[1][i]])@ \\
...
The 4x4, 16-channel histogram plots are given in Fig.~\ref{hists_text_4x4} for the inserted text and in Fig.~\ref{hists_bin_code_4x4} for the inserted binary code.   
...
Obviously, the histograms do not show anything close to the normal distribution. In both Fig.~\ref{hists_text_4x4} and Fig.~\ref{hists_bin_code_4x4} the $\chi^2$ values are in the range of hundreds, which is orders of magnitude above the critical value of $\chi^2_{cr} = 7.81$ at the level of significance 0.05.  
